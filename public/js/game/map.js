var Map = function(options) {
    help.mergeWithModel(this, options);
    this.map = help.asciiArtToMap(options.map, options.characterMap);
    help.finalizeTilemap(this);
    this.id = options.id;
};

Map.prototype = {
    id: null, // This is the object ID
    group: 'background',    // We use the 'backround' group we created above with our 'setGroups' call.
    map: null,
    backgroundImageUrl: null,
    _coverables: [],
    _previousDraws: [],
    characterMap: [
      [null, ' '],
      [0, '%'],
      [1, '-'],
      [2, '/'],
      [3, '\\'],
      [4, '+'],
      [5, '#'],
      [6, 'm'],
      [7, '['],
      [8, ']'],
      [9, '*']
    ],
    
    prepare: function() {
      gbox.createCanvas('map_canvas', { w: this.w, h: this.h });
      this._tilecanvas = gbox.getCanvas('map_canvas');
      gbox.blitTilemap(gbox.getCanvasContext('map_canvas'), this);
      this._bgimg = gbox.getImage(this.backgroundImageUrl);
    },
    
    blit: function() {
      var ctx = gbox.getBufferContext(),
          s = this._bgSize,
          chars = gbox.getGroup('players');

      var draw = {
        dx: 0, dy: 0, dw: this.w, dh: this.h,
        sourcecamera: true,
        parallaxx: 0.5, 
        parallaxy: 0.5
      };
      gbox.blit(ctx, this._bgimg, draw);
      gbox.blit(ctx, this._tilecanvas, draw);
      
      while(this._coverables.length > 0) {
        var draw = this._coverables.splice(0,1)[0];
        gbox.blit(ctx, this._bgimg, draw);
        gbox.blit(ctx, this._tilecanvas, draw);
      }

      while(this._previousDraws.length > 0) {
        var draw = this._previousDraws.splice(0,1)[0];
        gbox.blit(ctx, this._bgimg, draw);
        gbox.blit(ctx, this._tilecanvas, draw);
      }

      for(var i in chars) {
        var c = chars[i],
            draw = {
              x: c.x, y: c.y,
              w: c.w, h: c.h,
              dx: c.x, dy: c.y,
              dw: c.w, dh: c.h
            };
        this._previousDraws.push(draw);
        gbox.blit(ctx, this._bgimg, draw);
        gbox.blit(ctx, this._tilecanvas, draw);
      }

    },
    
    cover: function(item) {
      this._coverables.push({
        x: item.x, y: item.y,
        w: item.w, h: item.h,
        dx: item.x, dy: item.y,
        dw: item.w, dh: item.h
      });
    },
    
    tileIsSolid: function(t) {
      return this.tileIsSolidCeil(t) || tileIsSolidFloor(t);
    },
    
    tileIsNonSolid: function(t) {
      return this.tileIsClimbthroughFloor(t) || tileIsLadder(t);
    },

    tileIsSolidCeil: function(t) {
      return t !== null && ((t >= 1 && t<=4) || (t >=7 && t <=8));
    },
    
    tileIsSolidFloor: function(t) {
      return t !== null && ((t >= 1 && t<=4) || (t >=6 && t <=8));
    },
    
    tileIsClimbthroughFloor: function(t) {
      return t === 0;
    },
    
    tileIsLadder: function(t) {
      return t !== null && (t === 0 || t == 5);
    },
    
    tileIsGemOrigin: function(t) {
      return t !== null && t == 4;
    }
      
}
